...
class WeightLoader(object):
    ...
    def __init__(self, weights_file_path):
        ...
        # print(len(self.weights_file.read())) # 52604710
        # self.weight_pass = 0
    ...
    def _load_one_param_type(self, conv_params, param_category, suffix):
        ...
        'weights' or 'bias')
        """
        # print(conv_params.node_name, conv_params.conv_weight_dims ) # additional
        ...
        # self.weight_pass += param_size * 4 # additional
...
class GraphBuilderONNX(object):
    ...
    def build_onnx_graph(
        ...
        model_def = helper.make_model(self.graph_def, opset_imports=[helper.make_opsetid("", 10)],  # set opset to 13
                                      producer_name='darknet to ONNX example',
                                      ir_version=7)  # fix onnxruntime
        return model_def
    ...
    def _make_conv_node(self, layer_name, layer_dict):
        ...
        layer_dict -- a layer parameter dictionary (one element of layer_configs)
        """
        # print(layer_dict.keys()) # additional
...
def main(cfg_file='yolov4.cfg', weights_file='yolov4.weights', output_file='yolov4.onnx', strides=None, neck='PAN'):
    ...
    if not strides:
        return ValueError('strides is required')
    print('获取strides')

    output_tensor_dims = OrderedDict()
    print('创建output_tensor_dims')
    ...
    # Fix "not be treated as constant value/weight" warnings
    inputs = yolo_model_def.graph.input
    name_to_input = {}
    for input in inputs:
        name_to_input[input.name] = input

    for initializer in yolo_model_def.graph.initializer:
...
@app.post("/transfer")
async def transfer(cfg_file: UploadFile = File(...), weights_file: UploadFile = File(...)):
    cfg_file_path = "cfg_file.cfg"
    weights_file_path = "weights_file.weights"
    with open(cfg_file_path, "wb") as cfg_files:
        cfg_files.write(cfg_file.file.read())
    with open(weights_file_path, "wb") as weights_files:
        weights_files.write(weights_file.file.read())
    striders = []
    with open(cfg_file_path, "rb") as _cfg_files:
        lines = _cfg_files.readlines()
    for line in lines:
        if b'stride' in line:
            striders.append(int(line.split(b'=')[-1]))
    # print(striders)
    main(cfg_file=cfg_file_path, weights_file=weights_file_path, output_file="yolov4.onnx", strides=striders, neck='PAN')
    return True
